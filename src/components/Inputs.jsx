...
export function InputUploadGroupWithIcon({ label, width, onInput, value, className, labelClass, ref }) {
  const [fileName, setFileName] = useState("No file selected");
    ...
    <div className={`${width} flex items-end`}>
      <div className="flex flex-col w-full">
        ...
        <label className={`cursor-pointer p-2 w-full rounded flex gap-3 bg-white overflow-hidden hover:ring-2 hover:ring-violet-400 focus:outline-none transition ${className}`}>
          ...
          <input type="file" className="hidden" onChange={handleUpload} onInput={onInput} ref={ref} />
...
/*
export const InputUploadGroupWithIcon = forwardRef(({ label, width, onInput, className, labelClass }, ref) => {
  const [fileName, setFileName] = useState("No file selected");
  const [icon, setIcon] = useState(
    <svg width="20px" height="20px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      <mask id="mask0_1358_2896" style={{ maskType: "alpha" }} maskUnits="userSpaceOnUse" x="3" y="3" width="18" height="18">
  ...
  function handleUpload(e) {
    ...
    if (onInput) onInput(e); // propagate file to parent
  }
  ...
  return (
    <div className={`${width} flex items-end`}>
      <div className="flex flex-col w-full">
        ...
        <label className={`cursor-pointer p-2 w-full rounded flex gap-3 bg-white overflow-hidden hover:ring-2 hover:ring-violet-400 focus:outline-none transition ${className}`}>
          <span className="flex items-center">{icon}</span>
          <input type="file" className="hidden" onChange={handleUpload} ref={ref} />
          <span className="text-gray-800">{fileName}</span>
        </label>
      </div>
    </div>
